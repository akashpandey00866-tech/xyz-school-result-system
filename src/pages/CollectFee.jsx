...
function CollectFee({
  ...
  const academicDue = Math.max(0, Number(due || 0));
  const numericTransportDue = Math.max(0, Number(transportDue || 0));
  const numericDue = transportationMode ? numericTransportDue : academicDue;
  ...
  return (
    <section className="rounded-3xl border border-slate-200 bg-white p-5 sm:p-7 shadow-sm">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        ...
        <div>
          <div className="inline-flex items-center gap-2 rounded-full bg-emerald-50 px-3 py-1 text-xs font-bold text-emerald-700">
            <Receipt size={14} />
            {transportationMode ? "TRANSPORTATION COLLECTION" : "FEE COLLECTION"}
          </div>
      ...
      <div className="mt-6 grid grid-cols-2 lg:grid-cols-4 gap-3">
        ...
        <Summary
          title="Transport Due"
          value={money(numericTransportDue)}
          icon={<WalletCards size={17} />}
        />
